// Seed the catalog with 110 electronic products across 22 categories.
//
...
//   (products tagged attributes.seed = 'electronics' and the categories below),
//   so re-running is safe and never duplicates.
// • Set WIPE_ALL=true to clear the WHOLE catalog first (everything becomes
//   electronics only):  WIPE_ALL=true node scripts/seed-electronics.js
// • No product images are set — the storefront shows an "Image coming soon"
//   placeholder until a photo is uploaded from Admin → Products.
// • premium_price is auto-assigned from the price range: 99 / 199 / 299 / 499 / 999.
//
// It connects to whatever DB the server is configured for (DATABASE_URL if set,
// otherwise the local DB_HOST/DB_NAME). To seed production, run it in an
// environment where DATABASE_URL points at the production database.

import { randomUUID } from 'crypto';
import { sequelize } from '../src/db/index.js';
import { Section, Category, Product } from '../src/db/models/index.js';

const SECTION_NAME = 'Electronics Store';
const SECTION_ID = randomUUID();
const SEED_TAG = 'electronics';
const DEFAULT_WARRANTY = '1-year manufacturer warranty · 7-day replacement · 30-day returns';
const APPLIANCE_WARRANTY = '2-year comprehensive warranty (extended on compressor/panel) · easy returns';

// Premium offer price strictly from the price band.
function premiumFor(price) {
