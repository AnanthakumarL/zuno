...
import { DeliveryManagement } from '../../db/models/index.js';
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const { count, rows } = await DeliveryManagement.findAndCountAll({
...
router.post('/', async (req, res, next) => {
  try { res.status(201).json(await DeliveryManagement.create(req.body)); } catch (err) { next(err); }
});

router.get('/:id', async (req, res, next) => {
  try {
    const item = await DeliveryManagement.findByPk(req.params.id);
