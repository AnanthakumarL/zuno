...
import { ProductionManagement } from '../../db/models/index.js';
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const { count, rows } = await ProductionManagement.findAndCountAll({
...
router.post('/', async (req, res, next) => {
  try { res.status(201).json(await ProductionManagement.create(req.body)); } catch (err) { next(err); }
});

router.get('/:id', async (req, res, next) => {
  try {
    const item = await ProductionManagement.findByPk(req.params.id);
